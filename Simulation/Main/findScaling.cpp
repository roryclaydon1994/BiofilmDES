...
)
{
  ...
  // In microns / hr
  constexpr float cts {
    constants::baseTimeScale / constants::baseLengthScale
  };
  headers.push_back("RodGrowthRate");
  values.push_back(RodShapedBacterium::mAvgGrwthRate/cts);
...
)
{
  ...
  {
    const double tau { RodShapedBacterium::mAvgGrwthRate*times[ii] };
    out_file << lengths[ii] << '\t';
    out_file << tau         << '\t';
    out_file << nums[ii]    << '\n';
  }
...
void findScaling(float kappa, float bend_rig, float grwth_rate)
{
  ...
  constexpr uint n_trials { 20 };

  std::stringstream ss;
  ss << "kappa_" << kappa << "_B_" << bend_rig << "_mu_" << grwth_rate;
...
int main(int argc, char const *argv[])
{
  ...
  if ( argc==5 )
  {
    ...
    const double kappa        { std::stod( argv[2]) };     // Spring constant
    const double bend_rig     { std::stod( argv[3]) };     // Bending rigidity
    const double grwth_rate   { cts*std::stod( argv[4]) }; // Growth rate

    sim_out_dir += "/" + run_dir + "/";
    findScaling(kappa,bend_rig,grwth_rate);
  }
  else
  {
